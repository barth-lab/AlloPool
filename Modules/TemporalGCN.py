...
from Modules.tensor_typing import Float
...
class AttTGCN(nn.Module):
    ...
    def forward(
        ...
        probs = F.softmax(probs, dim=1).unsqueeze(
            -1
        )  # Provide node importance across time
        for period in range(self.periods):
            if edge_weight is not None:
                edge_attr = edge_weight[period]
            else:
                edge_attr = None
            H = self.base_tgcn(X[period, :, :], edge_index, edge_attr, H)
