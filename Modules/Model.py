...
class EdgePoolModel(nn.Module):
    ...
    def encode(
        ...
        x_time = torch.stack(
            unbatch(batch.x, batch=batch.batch), axis=0
        )  # [:-1] # T, N, d
        if pred_all:
    ...
    def kabsch_rmsd(
        ...
        if pred_coords.shape[0] != true_coords.shape[0]:
            raise ValueError(
                "pred pos and target pos must have the same number of points"
            )
        ...
        if scale:
            rmsd = torch.sqrt(
                ((true_coords_align * 10 - pred_coords * 10) ** 2).sum(axis=-1).mean()
            )
        else:
            rmsd = torch.sqrt(
                ((true_coords_align - pred_coords) ** 2).sum(axis=-1).mean()
            )
        return rmsd, [rmsd]

    def rmsd_loss(self, pred_coords, true_coords, align=True, scale=True):
        ...
        for i in range(pred_coords.shape[0]):
            if align:
                ...
                rmsd = torch.sqrt(
                    ((pred_coords[i] - true_coords[i]) ** 2).sum(axis=-1).mean()
                )
    ...
    def centered_loss(self, pred_coords):
        # pred_coords is shape [T,N,3]
    ...
    def contact_loss(
        ...
        distance_true = (
            einx.subtract("b i j, b n j -> b i n j", true_coords, true_coords).norm(
                dim=-1
            )
            * 10
        )
        ...
        if pred_coords.shape[-1] == 3:
            distance_pred = (
                einx.subtract("b i j, b n j -> b i n j", pred_coords, pred_coords).norm(
                    dim=-1
                )
                * 10
            )
