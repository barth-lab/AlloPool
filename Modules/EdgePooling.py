...
from Modules.tensor_typing import Float, Int
...
class EdgePoolBlock(nn.Module):
    def __init__(
        ...
        self.pair_attn = PairwiseAttention(dim=in_channels, heads=2, dim_head=64)
    ...
    def forward(
        ...
        if pair_attn is None:
            ...
            pair_attn = rearrange(ingoing, "b i -> b () i") + rearrange(
                outgoing, "b i -> () b i"
            )  # Shape [N_nodes, N_nodes, dim]
        ...
        if recycle_pair_attention:
            if pair_edge is None:
                pair_edge = torch.zeros(
                    pair_attn.shape[0], pair_attn.shape[0], 1, device=pair_attn.device
                )
...
class EdgePool(nn.Module):
    ...
    def forward(
        ...
        scores = (
            torch.ones(self.depth, edge_index.size(1)) * -torch.inf
        )  # To retrieve scores after
        ...
        for i, pooler in enumerate(self.pool_layer):
            (
                ...
                pair_edge,
                pair_attn,
            ) = pooler(  # Try put back x cause was out_pooling
                x=out_pooling,
                edge_index=edge_index,
                edge_attr=edge_attr,
                edge_index_perm=edge_map,
                pair_attn=pair_attn,
                pair_edge=pair_edge,
            )
