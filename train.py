...
class Logger():
    ...
    def __init__(self, name) -> None:
        ...
        self.edge_ref : List[Tensor] = [] # list of reference edges by batch, e.g. persistence edges
...
def get_model(args, device='cpu'):
    ...
    if args.checkpoint is not None: #args.checkpoint is None, '', or a checkpoint name
        ...
        if hasattr(model, "use_displ"): #temp fix
            model.pred_displ = model.use_displ
    else:
...
def get_dataset(sim_type, data_dir, use_cache=True, cache_name='', rotary_embed=None, replicates=1, timesteps=20, stride=1,
                threshold=1.5, first_frame = None, last_frame=None, batch_size=20, overlap=None, shuffle=True, persistence=0.5,
                normalize=False, test_set=False, test_n=1, drop_angles=False, drop_cbs=False,
                test_only=False,
                args = None) -> Tuple[PersistenceLoader, Normalizer, None|PersistenceLoader]: 
    '''
    Either sim_type or data_dir should be None. 
    If test set, returns last replicate as a test dataloader
    '''
    assert ((sim_type is None) ^ (data_dir is None)) or use_cache # sim_type xor data_dir is None
    cache_dir = path.join("cache",cache_name,)
    ...
    if use_cache :
        ...
        frames, frame_idx, repl_len, data_files = \
            apply_to_all_simulation(
                sim_path,replicates,timesteps,first_frame, last_frame,
                use_displacement=True, use_dihedral=not drop_angles,
        ...
        for i,n in enumerate(repl_len):
            repl_idx.extend([i]*n)
        data = make_dataset(coords,edges,weights, frame_idx=frame_idx, repl_idx=repl_idx)
    ...
    if test_set :
        assert test_n <= len(repl_len), f"test_n = {test_n} > replicates_n = {len(repl_len)}"
    ...
    loader = PersistenceLoader(data, repl_len, batch_size=batch_size,offset=overlap,shuffle=shuffle, threshold=persistence, stride=stride)  
    print('train set')
    ...
    if test_set:
        ...
        print(f'Average number of edges {np.mean([d.edge_index.shape[1] for d in data_test]):.0f}')
        test_only = test_only or (len(data_test) < 1000) # if dataset is small use all test set
        loader_test = PersistenceLoader(data_test,repl_test,batch_size=batch_size, stride=stride,
                                        offset=max(overlap, 1 if test_only else 20),# don't spend too much time on test set
                                        shuffle=False, threshold=persistence)
        print('Number of batches',len(loader_test))
    else:
        loader_test = None

    return loader, normalizer, loader_test,
...
def main(args, logger:Logger, data_args, device='cpu'):
    ...
    if os.path.isdir(tsb_path) and args.checkpoint is None:#empty directory of previous runs if necessary
        i= 0 
    ...
    dataloader, normalizer, test_dataloader = get_dataset(
        args.sim_type, args.dir, use_cache=not args.no_cache, rotary_embed = None,
        replicates = args.replicates, timesteps=args.timesteps, threshold=args.threshold,
        first_frame=args.first_frame, last_frame=args.last_frame,
        batch_size=args.batch_size, overlap=args.overlap, stride=args.stride, shuffle=args.shuffle,
        persistence=args.persistence, normalize=args.normalize, cache_name=args.cache_name,
        drop_angles=args.drop_angles, drop_cbs=args.drop_cbs,
        test_set=True, args=data_args
        )
    ...
    for epoch_i in progress:
        ...
        if not args.test_only:# train loop
            model.train()
            for batch_i,batch in tqdm(enumerate(dataloader), 'Train loop', len(dataloader)):
                if args.cuda:
                ...
                loss = all_loss["CA_bond_loss"] * args.rmsd_coef + all_loss["rmsd_loss"] * args.transl_coef + all_loss["contact_loss"] * args.contact_coef
        ...
        if test_dataloader is not None:
            ...
            with torch.no_grad():
                ...
                for batch_i, batch in tqdm(enumerate(test_dataloader),desc= 'Test loop', total=len(test_dataloader)):
                    if args.cuda:
                        batch = batch.to(device)
                    ...
                    loss = all_loss["rmsd_loss"] * args.rmsd_coef + all_loss["translation_loss"] * args.transl_coef + all_loss["contact_loss"]*args.contact_coef
...
if __name__=="__main__":
    ...
    parser.add_argument(
        ...
        help='If passed, create and save cache'
        )
    parser.add_argument(
        "--checkpoint",type=str,default=None, nargs='?', const='', help='Use saved model. \
    ...
    main_args_parser =  ArgumentParser(parents=[
            dataloder_parser, model_parser, training_parser, 
        ], conflict_handler='resolve')
    parser = ArgumentParser(parents=[
        dataloder_parser, dataset_parser, model_parser, training_parser, parser
    ], conflict_handler='resolve'
    )
    ...
    argdict = ArgDict()
    argdict.init_with_args(all_args,  ) #initialize args with flag where nothing was passed
    argdict.init_with_args(main_args, tags={'main'},)
    argdict.init_with_args(dataset_args, tags={'data'},)
